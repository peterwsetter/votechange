...
In this graph, the House vote total proportion for each party is plotted against Trump +/-, the representative's Trump-aligned voting relative to what it would be expected to be based on President Trump's performance in that district in 2016. Though highly variable, there appears to be a positively correlated trend between Trump +/- and Democratic vote total proportion. There also appears to be a weaker negatively correlated trend with Republican vote total proportion, though it may be noise.
...
I performed a simple linear regression, using Trump +/- as the predictor variable and vote total proportion as the dependent variable. A separate model was made for each party since we observed, and one would expect, opposite relationships.

The results of this regression support the claim that Democrats won because of greater voter enthuasism. Assuming the incumbant House member voted with President Trump exactly as one would expect based on the President's performance in 2016, Democratic vote total was `r d_baseline` compared to `r r_baseline` for Republican candidates.

Similarly, Trump +/- is nonportionally correlated with vote total proportion. Comparing the models, Trump +/- has a `r realative_effect` times greater effect on Democratic vote total proportion compared to Republician.
